...
function transform(source, options) {
  ...
  console.log('innerHTML',dom.window.document.documentElement.innerHTML);
  let templateRefsToAdd = []
  templateRefy(htmlRoot, attribute, search, templateBodyFunc, templateRefsToAdd)
  const newSource = htmlRoot.documentElement.documentElement.innerHTML
...
function templateRefy(htmlRoot, attribute, search, templateBodyFunc, templateRefsToAdd) {
  console.log('.')
  htmlRoot.childNodes.forEach((element, i)  => {
    console.log(htmlRoot.children[i])
    if (htmlRoot.children[i] && htmlRoot.children[i].hasAttribute instanceof Function && htmlRoot.children[i].hasAttribute(attribute)) {
      console.log('attribute found, value :', htmlRoot.children[i].getAttribute(attribute))
      console.log('search', search)
      // if somehow the value of the attribute is invalid, we skip
      const matched = htmlRoot.children[i].getAttribute(attribute).match(search);
      if (!matched) {
